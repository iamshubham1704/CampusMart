...
export function refreshToken(oldToken) {
  try {
    ...
    const newToken = jwt.sign(newPayload, JWT_SECRET, { 
      expiresIn: decoded.role === 'admin' ? '7d' : '30d' 
    });
...
export const getStoredToken = (userType = null) => {
  ...
  if (userType === 'buyer') {
    return localStorage.getItem('buyerToken') || localStorage.getItem('auth-token');
  } else if (userType === 'seller') {
    return localStorage.getItem('token') || localStorage.getItem('sellerToken') || localStorage.getItem('auth-token');
  } else if (userType === 'admin') {
    return localStorage.getItem('adminToken') || localStorage.getItem('admin-auth-token');
  }
